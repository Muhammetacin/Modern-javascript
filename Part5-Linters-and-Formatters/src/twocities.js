...
const showCity1Name =
  document.getElementById("inputArea").children[0].children[0];
const showCity2Name =
  document.getElementById("inputArea").children[2].children[0];
...
async function getCity5Days(cityName1, cityName2) {
  const apiString5DaysCity1 =
    "https://api.openweathermap.org/data/2.5/forecast?q=" +
    cityName1 +
    "&cnt=40&units=metric&appid=" +
    APIkey;
  const response5DaysCity1 = await fetch(apiString5DaysCity1).then((response) =>
    response.json()
  );

  const apiString5DaysCity2 =
    "https://api.openweathermap.org/data/2.5/forecast?q=" +
    cityName2 +
    "&cnt=40&units=metric&appid=" +
    APIkey;
  const response5DaysCity2 = await fetch(apiString5DaysCity2).then((response) =>
    response.json()
  );

  if (
    response5DaysCity1.city == undefined &&
    response5DaysCity2.city != undefined
  ) {
  ...
  visitedCities1 = visitedCities1.filter(
    (value, index, city) => city.indexOf(value) === index
  );

  visitedCities2.push(responseTempValuesCity2[0]);
  visitedCities2 = visitedCities2.filter(
    (value, index, city) => city.indexOf(value) === index
  );
...
function setTempAllDays(tempData1) {
  ...
  for (let i = 0; i < dayNamesOfWeek1.children.length; i++) {
    dayNamesOfWeek1.children[i].children[2].innerHTML =
      Math.round(tempData1[i + 2]) + "°";
...
function setTempAllDays2(tempData2) {
  ...
  for (let i = 0; i < dayNamesOfWeek2.children.length; i++) {
    dayNamesOfWeek2.children[i].children[2].innerHTML =
      Math.round(tempData2[i + 2]) + "°";
