...
export const setTemperatureOfDays = (temperatureData) => {
  ...
  for (let i = 0; i < dayNamesOfWeek.children.length; i++) {
    dayNamesOfWeek.children[i].children[2].innerHTML =
      Math.round(temperatureData[i + 1]) + "°";
...
export const setWeatherDescriptionOfDays = (weatherDescription) => {
  ...
  for (let i = 0; i < dayNamesOfWeek.children.length; i++) {
    dayNamesOfWeek.children[i].children[1].innerHTML =
      weatherDescription[i + 1];
  }
};

export const setCityProperties = (
  responseCityName,
  responseTemperatureValues,
  weatherDescription
) => {
