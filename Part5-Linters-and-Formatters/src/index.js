...
const executeInput = () => {
  ...
  fetchCity.then((cityObject) =>
    setCityProperties(
      cityObject.responseCityName,
      cityObject.responseTemperatureValues,
      cityObject.weatherDescription
    )
  );
  fetchCity.then((cityObject) =>
    createGraphWithValues(cityObject.responseTemperatureValues)
  );
  fetchCity.then((cityObject) =>
    showVisitedCities(cityObject.responseCityName)
  );
};
